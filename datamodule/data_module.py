...
from .av_dataset import ContrastiveAVDataset
...
class DataModule(LightningDataModule):
    ...
    def test_dataloader(self):
        ...
        return dataloader
# （既存に似た pad 関数があってもOK。名前を被らせないために別名で定義）
def _pad_time_first_contrastive(batch_tensors, pad_val=0.0):
    """[T,C,H,W] を右パディングして [B,Tmax,C,H,W] に揃える"""
    lengths = [x.shape[0] for x in batch_tensors]
    Tmax = max(lengths)
    B = len(batch_tensors)
    C, H, W = batch_tensors[0].shape[1:]
    out = batch_tensors[0].new_full((B, Tmax, C, H, W), pad_val)
...
def collate_pad(batch):
    """
    ContrastiveAVDataset 用の collate:
      - video1 / video2 を時間方向で右パディング
      - targets を pad_sequence（padding_value=-1）
      - TextGrid をリストとして保持
    """
    v1 = [b["video1"] for b in batch]
    v2 = [b["video2"] for b in batch]
    tgs = [b["tg"] for b in batch]
    targets = [b["target"] for b in batch]

    v1s, len1 = _pad_time_first_contrastive(v1, 0.0)  # [B,T1max,1,H,W], [B]
    v2s, len2 = _pad_time_first_contrastive(v2, 0.0)  # [B,T2max,1,H,W], [B]

    targets_pad = torch.nn.utils.rnn.pad_sequence(
...
class ContrastiveDataModule(LightningDataModule):
    """
    既存の DataModule はそのまま残し、pre-contrastive 時だけこのクラスを使う。
    """
    def __init__(
        self,
        args=None,
    ...
    def train_dataloader(self):
        dataset = ContrastiveAVDataset(
            root_dir=self.args.root_dir,
            # 対照ペア CSV は args.pair_train_file を直指定（labels/ 決め打ちはしない）
            label_path=self.args.pair_train_file or os.path.join(self.args.root_dir, "labels", self.args.train_file),
            subset="train",
            modality=self.args.modality,      # "video" 前提
            audio_transform=None,             # audio 2入力は未実装なので None
            video_transform=VideoTransform("train"),
        )

        # 既存のバケツローダを使っているならそれを踏襲
        try:
            from datamodule.bucket import CustomBucketDataset
            dataset = CustomBucketDataset(
                dataset,
                dataset.input_lengths,
                getattr(self.args, "max_frames", 1000),
                self.train_num_buckets,
                batch_size=self.batch_size,
            )
    ...
    def val_dataloader(self):
        if getattr(self.args, "pair_val_file", None) is None and getattr(self.args, "val_file", None) is None:
            return None
        dataset = ContrastiveAVDataset(
            root_dir=self.args.root_dir,
            label_path=self.args.pair_val_file or os.path.join(self.args.root_dir, "labels", self.args.val_file),
            subset="val",
            modality=self.args.modality,
    ...
    def test_dataloader(self):
        if getattr(self.args, "pair_test_file", None) is None and getattr(self.args, "test_file", None) is None:
            return None
        dataset = ContrastiveAVDataset(
            root_dir=self.args.root_dir,
            label_path=self.args.pair_test_file or os.path.join(self.args.root_dir, "labels", self.args.test_file),
            subset="test",
            modality=self.args.modality,
            audio_transform=None,
            video_transform=VideoTransform("test"),
